...
rootDir = windowsShortPath(os.getcwd())
libsDir = os.path.join(rootDir, libsLoc)
thirdPartyDir = os.path.join(rootDir, 'ThirdParty')
usedPrefix = os.path.join(libsDir, 'local')
...
def run(commands):
    ...
    if win:
        ...
        result = subprocess.run(
            ...
            shell=True,
            env=modifiedEnv,
            cwd=windowsShortPath(os.getcwd()),
        ).returncode == 0
...
win:
    ...
    git apply %ROOT_DIR%\\source\\Telegram\\build\\prepare\\tg_angle-space-path.patch
    cmake -B out ^
