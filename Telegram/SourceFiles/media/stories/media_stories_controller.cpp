...
void Controller::maybeSuggestGhostMode(not_null<Data::Story*> story) {
	if (_ghostPromptOffered || story->peer()->isSelf()) {
		return;
	}
	const auto &ghost = XGramSettings::ghost(&story->session());
	if (!ghost.suggestGhostModeBeforeViewingStory()
		|| ghost.isGhostModeActive()) {
		return;
	}

	_ghostPromptOffered = true;
	_ghostPromptPending = true;
	const auto weak = make_weak(this);
	const auto finish = [=](bool enable) {
		if (const auto strong = weak.get()) {
			strong->_ghostPromptPending = false;
			if (enable && strong->_session) {
				XGramSettings::ghost(strong->_session).setGhostModeEnabled(true);
			}
		}
	};
	uiShow()->show(Ui::MakeConfirmBox({
		.text = tr::xgram_SuggestGhostModeBeforeViewingStoryDescription(),
		.confirmed = [=] { finish(true); },
		.cancelled = [=] { finish(false); },
		.confirmText = tr::xgram_GhostModeToggle(),
		.cancelText = tr::lng_cancel(),
		.title = tr::xgram_SuggestGhostModeBeforeViewingStory(),
	}));
}
...
void Controller::markAsRead() {
	...
	if (_viewed || _ghostPromptPending) {
		return;
	}
	_viewed = true;
