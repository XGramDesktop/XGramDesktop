...
bool OverlayWidget::hasCopyMediaRestriction(bool skipPremiumCheck) const {
	...
	// XGram: removed; allow downloading any stories
	return false;
}
...
bool OverlayWidget::streamingRequiresControls() const {
	return !_stories
		&& _document;
	// XGram: allow vieo messages seeking
	//  && (!_document->isAnimation() || _document->isVideoMessage());
}
...
void OverlayWidget::activate() {
	...
	if (XGramFeatures::StreamerMode::isEnabled()) {
		XGramFeatures::StreamerMode::hideWidgetWindow(_window);
	} else {
		XGramFeatures::StreamerMode::showWidgetWindow(_window);
	}
}
...
void OverlayWidget::clearBeforeHide() {
	XGramState::disableGhostModeOnStoryClose(_storiesSession);
	_message = nullptr;
	_sharedMedia = nullptr;
	_sharedMediaData = std::nullopt;
	_sharedMediaDataKey = std::nullopt;
