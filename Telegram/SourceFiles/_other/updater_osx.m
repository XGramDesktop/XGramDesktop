...
void RemoveQuarantineFromBundle(NSString *path) {
	...
	RemoveQuarantineAttribute([path stringByAppendingString:@"/Contents/MacOS/XGram"]);
...
int main(int argc, const char * argv[]) {
	...
	if (update) {
		...
		// Take the Updater (this currently running binary) from the place where it was placed by XGram
		// and copy it to the folder with the new version of the app (ready),
		// so it won't be deleted when we will clear the "XGram.app/Contents" folder.
