...
class Suggestions;
class AccountHubList;
class GlobalPinsModule;
class ChatSearchIn;
...
void ShowAccountHub();
void HideAccountHub();
[[nodiscard]] rpl::producer<bool> AccountHubModeValue();
...
private:
	...
	void updateAccountHubMode(bool active);
	void refreshTopBars();
	...
	object_ptr<Ui::RpWidget> _cryptoTickerWrap;
	object_ptr<Ui::FlatLabel> _cryptoTicker;
	std::unique_ptr<XGram::CryptoTicker> _cryptoTickerSource;
	base::unique_qptr<Ui::PopupMenu> _cryptoTickerMenu;
	...
	object_ptr<Ui::ElasticScroll> _accountHubScroll;
	AccountHubList *_accountHub = nullptr;
	GlobalPinsModule *_globalPinsModule = nullptr;
	bool _accountHubActive = false;
	bool _accountHubScrollRestoring = false;
