...
rpl::variable<bool> GlobalAccountHubMode = false;
int GlobalAccountHubScrollTop = 0;
base::unique_qptr<Ui::PopupMenu> GlobalAccountHubMenu;
...
[[nodiscard]] CryptoTickerTheme CurrentCryptoTickerTheme() {
	const auto symbol = XGramSettings::getInstance().cryptoTickerSymbol();
	auto accent = (symbol == u"BTC"_q)
		? QColor(u"#F7931A"_q)
		: (symbol == u"ETH"_q)
		? QColor(u"#7C83F7"_q)
		: (symbol == u"TON"_q)
	...
	finish.setAlpha(18);
	border.setAlpha(155);
	return { start, finish, border, accent };
}
...
void ShowAccountHub() {
	GlobalAccountHubMode = true;
}

void HideAccountHub() {
	GlobalAccountHubMode = false;
}

rpl::producer<bool> AccountHubModeValue() {
	return GlobalAccountHubMode.value();
}

struct MergedDialogEntry {
	not_null<Main::Account*> account;
	not_null<Row*> row;
};

[[nodiscard]] int MergedDialogRowHeight(const MergedDialogEntry &entry) {
	return Row::ComputeSt(entry.row->entry(), FilterId()).height;
}

void OpenMergedDialog(
		not_null<Window::SessionController*> controller,
		const MergedDialogEntry &entry,
		bool leaveHub) {
	const auto history = entry.row->history();
	if (!history) {
		return;
	}
	...
	controller->window().invokeForSessionController(
		entry.account,
		peer,
		[=](not_null<Window::SessionController*> target) {
			target->showPeerHistory(peer);
		});
}

void OpenMergedDialogByLink(
		not_null<Window::SessionController*> controller,
		const MergedDialogEntry &entry) {
	const auto history = entry.row->history();
	if (!history) {
	...
	if (!username.isEmpty()) {
		// Resolve the public t.me identity in the currently visible session.
		// This opens the channel/profile in the main XGram window and keeps the
		// current account list in place instead of spawning a separate window.
		controller->showPeerByLink(Window::PeerByLinkInfo{
			.usernameOrId = username,
		});
		return;
	}
	if (const auto user = peer->asUser(); user && !user->phone().isEmpty()) {
		controller->showPeerByLink(Window::PeerByLinkInfo{
			.phone = user->phone(),
		});
		return;
	}
	// Private peers do not have a transferable public link. Open them in the
	// owning session, still in the same main window.
	OpenMergedDialog(controller, entry, false);
}

void PaintMergedDialog(
		Painter &p,
		const MergedDialogEntry &entry,
		int top,
		int width,
		bool selected,
		not_null<Window::SessionController*> controller) {
	const auto height = MergedDialogRowHeight(entry);
	const auto &rowStyle = Row::ComputeSt(entry.row->entry(), FilterId());
	const auto background = selected ? st::dialogsBgOver : st::dialogsBg;
	p.fillRect(0, top, width, height, background);
	p.save();
	p.translate(0, top);
	...
	Ui::RowPainter::Paint(p, entry.row, nullptr, context);
	p.restore();
}

void ShowMergedDialogMenu(
		not_null<Window::SessionController*> controller,
		const MergedDialogEntry &entry,
	...
	controller->window().invokeForSessionController(
		...
		peer,
		[=](not_null<Window::SessionController*> target) {
			GlobalAccountHubMenu = base::make_unique_q<Ui::PopupMenu>(
				target->widget(),
				st::popupMenuExpandedSeparator);
			const auto addAction = Ui::Menu::CreateAddActionCallback(
				GlobalAccountHubMenu);
			Window::FillDialogsEntryMenu(
				...
				addAction);
			if (GlobalAccountHubMenu->empty()) {
				GlobalAccountHubMenu = nullptr;
			} else {
				GlobalAccountHubMenu->popup(globalPosition);
			}
		});
}

class AccountHubList final : public Ui::RpWidget {
public:
	AccountHubList(
		QWidget *parent,
		not_null<Window::SessionController*> controller)
	...
	, _accountsCount(Core::App().domain().accountsAuthedCount()) {
		setMouseTracking(true);
		watchAccounts();
		refresh();
		Core::App().domain().unreadBadgeChanges(
		) | rpl::on_next([=] {
			refresh();
		}, lifetime());
	...
	void refresh() {
		...
		for (const auto account : Core::App().domain().orderedAccounts()) {
			...
			append(_main, account, data.chatsList()->indexed());
			if (const auto folder = data.folderLoaded(Data::Folder::kId)) {
				append(_archive, account, folder->chatsList()->indexed());
			}
		}
		const auto byDate = [](const auto &a, const auto &b) {
			return a.row->sortKey(FilterId()) > b.row->sortKey(FilterId());
		};
		ranges::stable_sort(_main, byDate);
		ranges::stable_sort(_archive, byDate);
...
protected:
	void paintEvent(QPaintEvent *e) override {
		...
		if (_main.empty() && _archive.empty()) {
			p.setFont(st::normalFont);
			p.setPen(st::windowSubTextFg);
			p.drawText(rect(), Qt::AlignCenter, tr::xgram_AccountHubEmpty(tr::now));
			return;
		}
		auto top = 0;
		for (auto i = 0; i != int(_main.size()); ++i) {
			const auto height = MergedDialogRowHeight(_main[i]);
			if (QRect(0, top, width(), height).intersects(e->rect())) {
				PaintMergedDialog(
					p,
					_main[i],
					top,
		...
		if (!_archive.empty()) {
			paintHeader(p, top, tr::lng_archived_name(tr::now), _archiveExpanded);
			top += kHeaderHeight;
			if (_archiveExpanded) {
				for (auto i = 0; i != int(_archive.size()); ++i) {
					const auto height = MergedDialogRowHeight(_archive[i]);
					if (QRect(0, top, width(), height).intersects(e->rect())) {
						PaintMergedDialog(
							p,
							_archive[i],
	...
	void mouseMoveEvent(QMouseEvent *e) override {
		const auto selected = indexAt(e->pos().y());
		if (_selected != selected) {
			_selected = selected;
			setCursor(selected >= 0 ? style::cur_pointer : style::cur_default);
			update();
		}
	}
	...
	void mousePressEvent(QMouseEvent *e) override {
		...
		const auto mainHeight = entriesHeight(_main);
		if (!_archive.empty()
			&& y >= mainHeight
			&& y < mainHeight + kHeaderHeight) {
			_archiveExpanded = !_archiveExpanded;
			resize(width(), desiredHeight());
			update();
			return;
		}
		const auto index = indexAt(y);
		if (index < 0) {
			return;
		}
		if (index < int(_main.size())) {
			OpenMergedDialog(_controller, _main[index], false);
		} else {
			OpenMergedDialog(
				_controller,
				_archive[index - int(_main.size())],
				false);
		}
	}

	void contextMenuEvent(QContextMenuEvent *e) override {
		const auto index = indexAt(e->pos().y());
		if (index < 0) {
			return;
		}
		const auto &entry = (index < int(_main.size()))
			? _main[index]
			: _archive[index - int(_main.size())];
		_selected = index;
		update();
		ShowMergedDialogMenu(_controller, entry, e->globalPos());
		e->accept();
	}
...
private:
	static constexpr auto kHeaderHeight = 42;
	...
	void checkAccountsCount() {
		const auto count = Core::App().domain().accountsAuthedCount();
		if (count > _accountsCount) {
			// Telegram activates the newly authorised account itself. Leaving
			// hub mode here keeps both the header and the chat list in sync.
			HideAccountHub();
		}
	...
	void append(
			std::vector<MergedDialogEntry> &to,
			not_null<Main::Account*> account,
			not_null<IndexedList*> list) {
		for (const auto row : *list) {
			if (row->history()) {
				to.push_back({ account, row });
			}
		}
	}

	[[nodiscard]] int entriesHeight(
			const std::vector<MergedDialogEntry> &entries) const {
		auto result = 0;
		for (const auto &entry : entries) {
			result += MergedDialogRowHeight(entry);
		}
		return result;
	}
	...
	[[nodiscard]] int desiredHeight() const {
		return std::max(
			1,
			entriesHeight(_main)
				+ (_archive.empty() ? 0 : kHeaderHeight)
				+ (_archiveExpanded ? entriesHeight(_archive) : 0));
	}

	[[nodiscard]] int indexAt(int y) const {
		auto top = 0;
		for (auto i = 0; i != int(_main.size()); ++i) {
			top += MergedDialogRowHeight(_main[i]);
			if (y < top) {
				return (y >= 0) ? i : -1;
			}
		}
		if (_archive.empty() || y < top + kHeaderHeight || !_archiveExpanded) {
			return -1;
		}
		top += kHeaderHeight;
		for (auto i = 0; i != int(_archive.size()); ++i) {
			top += MergedDialogRowHeight(_archive[i]);
			if (y < top) {
				return int(_main.size()) + i;
			}
		}
		return -1;
	}
	...
	void paintHeader(
			...
			const QString &text,
			bool expanded) const {
		p.fillRect(0, top, width(), kHeaderHeight, st::dialogsBg);
		p.setFont(st::semiboldFont);
		p.setPen(st::windowActiveTextFg);
		p.drawText(
			QRect(16, top, width() - 50, kHeaderHeight),
			Qt::AlignVCenter | Qt::AlignLeft,
			text);
		p.setPen(st::windowSubTextFg);
		p.drawText(
			QRect(width() - 40, top, 24, kHeaderHeight),
			Qt::AlignCenter,
			expanded ? QString::fromUtf8("\xE2\x96\xB2") : QString::fromUtf8("\xE2\x96\xBC"));
	}

	const not_null<Window::SessionController*> _controller;
	std::vector<MergedDialogEntry> _main;
	std::vector<MergedDialogEntry> _archive;
...
class GlobalPinsModule final : public Ui::RpWidget {
public:
	GlobalPinsModule(
		QWidget *parent,
		not_null<Window::SessionController*> controller,
	...
	, _geometryChanged(std::move(geometryChanged)) {
		setMouseTracking(true);
		_expanded = XGramSettings::getInstance().globalPinsExpanded();
		refresh(false);
		rpl::merge(
			Core::App().domain().unreadBadgeChanges(),
			Core::App().domain().accountsChanges(),
			XGramSettings::getInstance().globalPinsChanges()
		) | rpl::on_next([=] {
			refresh();
		}, lifetime());
	}

	void setArchiveMode(bool archiveMode) {
		if (_archiveMode == archiveMode) {
			return;
		}
	...
	[[nodiscard]] int desiredHeight() const {
		if (!_contextVisible
			|| _entries.empty()
			|| !XGramSettings::getInstance().globalPinsEnabled()) {
			return 0;
		}
		auto result = kHeaderHeight;
		if (_expanded) {
	...
	void refresh(bool notifyGeometry = true) {
		_entries.clear();
		const auto &pins = XGramSettings::getInstance().globalPinnedChats();
		for (const auto account : Core::App().domain().orderedAccounts()) {
			if (!account->sessionExists()) {
				continue;
			}
			const auto found = pins.find(account->session().userId().bare);
			if (found == pins.end()) {
				continue;
			}
			for (const auto peerId : found->second) {
				const auto history = account->session().data().history(PeerId(peerId));
				const auto folder = history->folder();
				if (bool(folder) != _archiveMode) {
					continue;
				}
				const auto list = folder
					? folder->chatsList()->indexed()
					: account->session().data().chatsList()->indexed();
				const auto row = list->getRow(Key(history));
				if (row) {
					_entries.push_back({ account, row });
				}
			}
		}
		ranges::stable_sort(_entries, [](const auto &a, const auto &b) {
			return a.row->sortKey(FilterId()) > b.row->sortKey(FilterId());
		});
		const auto height = desiredHeight();
		resize(width(), height);
		setVisible(height > 0);
		if (notifyGeometry) {
			_geometryChanged();
...
protected:
	void paintEvent(QPaintEvent *e) override {
		...
		p.setPen(st::windowActiveTextFg);
		p.drawText(
			QRect(16, 0, width() - 70, kHeaderHeight),
			Qt::AlignVCenter | Qt::AlignLeft,
			tr::xgram_GlobalPinsHeader(tr::now));
		p.setPen(st::windowSubTextFg);
		p.drawText(
			QRect(width() - 58, 0, 22, kHeaderHeight),
			Qt::AlignCenter,
			QString::number(_entries.size()));
		p.drawText(
			QRect(width() - 35, 0, 20, kHeaderHeight),
			Qt::AlignCenter,
			_expanded ? QString::fromUtf8("\xE2\x96\xB2") : QString::fromUtf8("\xE2\x96\xBC"));
		if (!_expanded) {
			return;
		}
		auto top = kHeaderHeight;
		for (auto i = 0; i != int(_entries.size()); ++i) {
			const auto height = MergedDialogRowHeight(_entries[i]);
			if (QRect(0, top, width(), height).intersects(e->rect())) {
				PaintMergedDialog(
					p,
					_entries[i],
	...
	void mousePressEvent(QMouseEvent *e) override {
		...
		if (e->pos().y() < kHeaderHeight) {
			_expanded = !_expanded;
			XGramSettings::getInstance().setGlobalPinsExpanded(_expanded);
			resize(width(), desiredHeight());
			_geometryChanged();
			update();
			return;
		...
		const auto index = indexAt(e->pos().y());
		if (index >= 0) {
			OpenMergedDialogByLink(_controller, _entries[index]);
		}
	}
	...
	void contextMenuEvent(QContextMenuEvent *e) override {
		...
		_selected = index;
		update();
		ShowMergedDialogMenu(_controller, _entries[index], e->globalPos());
		e->accept();
	}
...
private:
	static constexpr auto kHeaderHeight = 38;

	[[nodiscard]] int indexAt(int y) const {
		if (!_expanded || y < kHeaderHeight) {
			return -1;
		}
		auto top = kHeaderHeight;
		for (auto i = 0; i != int(_entries.size()); ++i) {
			top += MergedDialogRowHeight(_entries[i]);
			if (y < top) {
				return i;
			}
		}
	...
	const not_null<Window::SessionController*> _controller;
	const Fn<void()> _geometryChanged;
	std::vector<MergedDialogEntry> _entries;
	int _selected = -1;
	bool _expanded = true;
...
, _cryptoTickerWrap(_searchControls)
, _cryptoTicker(_cryptoTickerWrap, QString(), st::defaultFlatLabel)
, _chooseFromUser(
	_searchControls,
...
, _singleMessageSearch(&controller->session()) {
	...
	_scroll->setOverscrollTypes(
		...
		OverscrollType::Real);
	_accountHubScroll->setOverscrollTypes(
		OverscrollType::Real,
		OverscrollType::Real);
	_innerList = _scroll->setOwnedWidget(
		object_ptr<Ui::VerticalLayout>(this));
	_globalPinsModule = _innerList->add(object_ptr<GlobalPinsModule>(
		_innerList,
		controller,
	...
	_accountHub = _accountHubScroll->setOwnedWidget(
		object_ptr<AccountHubList>(this, controller));
	_accountHubScroll->widthValue() | rpl::on_next([=](int width) {
		_accountHub->resize(width, _accountHub->height());
		_accountHub->refresh();
	}, _accountHub->lifetime());
	_accountHubScroll->scrollTopValue(
	) | rpl::on_next([=](int top) {
		if (_accountHubActive && !_accountHubScrollRestoring) {
			GlobalAccountHubScrollTop = top;
		}
	}, lifetime());
	_accountHubScroll->hide();
	if (_layout == Layout::Main) {
		AccountHubModeValue(
		) | rpl::on_next([=](bool active) {
			updateAccountHubMode(active);
		}, lifetime());
	}
	_scrollToTop->raise();
	_lockUnlock->toggle(false, anim::type::instant);
	_cryptoTickerWrap->hide();
	_cryptoTicker->show();
	// The label fills most of the ticker and can otherwise consume a context
	// menu event after the ticker has been hidden and shown again. Keep all
	// pointer handling on the persistent wrapper where the menu filter lives.
	_cryptoTicker->setAttribute(Qt::WA_TransparentForMouseEvents);
	_cryptoTickerWrap->paintRequest(
	) | rpl::on_next([=](const QRect &clip) {
		auto p = Painter(_cryptoTickerWrap);
		p.setClipRect(clip);
		p.setRenderHint(QPainter::Antialiasing);
		const auto rect = QRectF(_cryptoTickerWrap->rect()).marginsRemoved(
			QMarginsF(1., 3., 1., 3.));
		const auto theme = CurrentCryptoTickerTheme();
		auto gradient = QLinearGradient(rect.topLeft(), rect.topRight());
		gradient.setColorAt(0., theme.start);
		gradient.setColorAt(1., theme.finish);
		p.setPen(QPen(theme.border, style::DevicePixelRatio()));
		p.setBrush(gradient);
		p.drawRoundedRect(rect, rect.height() / 2., rect.height() / 2.);
		p.setPen(Qt::NoPen);
		p.setBrush(theme.accent);
		const auto dot = std::max(6, _cryptoTickerWrap->height() / 5);
		p.drawEllipse(
			10,
			(_cryptoTickerWrap->height() - dot) / 2,
			dot,
			dot);
	}, _cryptoTickerWrap->lifetime());
	if (_layout == Layout::Main) {
		_cryptoTickerSource = std::make_unique<XGram::CryptoTicker>(this);
		_cryptoTickerSource->textValue(
		) | rpl::on_next([=](const QString &text) {
			_cryptoTicker->setText(text);
			updateControlsGeometry();
		}, lifetime());

		const auto &settings = XGramSettings::getInstance();
		settings.cryptoTickerEnabledValue(
		) | rpl::on_next([=](bool enabled) {
			if (enabled) {
				_cryptoTickerSource->refresh();
			}
			updateControlsGeometry();
		}, lifetime());
		settings.cryptoTickerSymbolChanges(
		) | rpl::on_next([=] {
			const auto theme = CurrentCryptoTickerTheme();
			_cryptoTicker->setTextColorOverride(theme.accent);
			_cryptoTickerWrap->update();
			_cryptoTickerSource->refresh();
		}, lifetime());
		_cryptoTicker->setTextColorOverride(CurrentCryptoTickerTheme().accent);
		base::install_event_filter(
			...
			[=](not_null<QEvent*> event) {
				...
				const auto context = static_cast<QContextMenuEvent*>(event.get());
				_cryptoTickerMenu = base::make_unique_q<Ui::PopupMenu>(
					_cryptoTickerWrap,
					st::popupMenuExpandedSeparator);
				auto symbols = std::vector<QString>{
					u"BTC"_q,
					u"ETH"_q,
					u"TON"_q,
					u"SOL"_q,
				...
				for (const auto &symbol : symbols) {
					submenu->addAction(symbol, [=] {
						XGramSettings::getInstance().setCryptoTickerSymbol(symbol);
					});
				}
				_cryptoTickerMenu->addAction(
					tr::xgram_CryptoTickerChange(tr::now),
					std::move(submenu));
				_cryptoTickerMenu->addAction(
					tr::xgram_CryptoTickerHide(tr::now),
					[] {
						XGramSettings::getInstance().setCryptoTickerEnabled(false);
					},
					&st::menuIconDelete);
				_cryptoTickerMenu->popup(context->globalPos());
...
void Widget::chosenRow(const ChosenRow &row) {
	...
	if (topicJump) {
		...
		&& !history->peer->isSelf()
		&& !XGramSettings::getInstance().disableStories()) {
...
void Widget::updateControlsVisibility(bool fast) {
	...
	const auto listVisible = !_suggestions && _hidingSuggestions.empty();
	_scroll->setVisible(listVisible && !_accountHubActive);
	_accountHubScroll->setVisible(listVisible && _accountHubActive);
	_globalPinsModule->setArchiveMode(_openedFolder != nullptr);
	const auto pinsVisible = listVisible
		&& !_accountHubActive
		&& !_openedForum
		&& !_openedCommunity
		&& _searchState.query.isEmpty();
	_globalPinsModule->setContextVisible(pinsVisible);
	updateStoriesVisibility();
...
void Widget::updateAccountHubMode(bool active) {
	if (_layout != Layout::Main || _accountHubActive == active) {
		return;
	}
	_accountHubActive = active;
	if (active) {
		const auto restoreTop = GlobalAccountHubScrollTop;
		_accountHubScrollRestoring = true;
		_accountHub->refresh();
		base::call_delayed(0, this, [=] {
			_accountHubScroll->scrollToY(restoreTop);
			_accountHubScrollRestoring = false;
		});
	} else {
		_accountHubScrollRestoring = false;
	}
...
void Widget::updateControlsGeometry() {
	...
	const auto tickerEnabled = _cryptoTickerSource
		&& XGramSettings::getInstance().cryptoTickerEnabled();
	const auto animatedTickerWidth = tickerEnabled
		? anim::interpolate(144, 0, narrowRatio)
		: 0;
	const auto cryptoTickerWidth = std::clamp(
		animatedTickerWidth,
		0,
		std::max(0, filterWidth - 80));
	const auto showCryptoTicker = (cryptoTickerWidth > 0);
	const auto searchWidth = filterWidth - cryptoTickerWidth;
	_search->setGeometryToLeft(
		filterLeft,
		filterTop,
		searchWidth,
		_search->height());
	_cryptoTickerWrap->setVisible(showCryptoTicker);
	if (showCryptoTicker) {
		_cryptoTickerWrap->setGeometryToLeft(
			filterLeft + searchWidth,
			filterTop,
			cryptoTickerWidth,
			_search->height());
		const auto labelLeft = 24;
		_cryptoTicker->resizeToWidth(
			std::max(1, cryptoTickerWidth - labelLeft - 8));
		_cryptoTicker->setVisible(cryptoTickerWidth > 54);
		_cryptoTicker->moveToLeft(
			labelLeft,
			(_cryptoTickerWrap->height() - _cryptoTicker->height()) / 2);
	}
	...
	auto right = filterLeft + searchWidth;
	...
	_updateScrollGeometryCached = [=] {
		...
		const auto scrollTop = chatFiltersTop
			+ ((_chatFilters
				...
				: 0);
		const auto scrollTopValue = int(scrollTop);
		const auto contentTop = scrollTopValue;
		const auto scrollHeight = std::max(
			0,
			height() - contentTop - bottomSkip);
		const auto wasScrollHeight = _scroll->height();
		_scroll->setGeometry(0, contentTop, scrollWidth, scrollHeight);
		_accountHubScroll->setGeometry(
			0,
			contentTop,
			scrollWidth,
			scrollHeight);
