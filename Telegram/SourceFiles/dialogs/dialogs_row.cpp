...
QRect CornerBadgeTTLRect(int photoSize) {
	return XGramUserpic::OnlineBadgeRect(photoSize, st::dialogsTTLBadgeSize);
}
...
void Row::PaintCornerBadgeFrame(
	...
	if (storiesCount) {
		...
		if (peer && (peer->forum() || peer->monoforum())) {
			...
			const auto radius = singleRadius
				? XGramUserpic::ComputeRadiusF(context.st->photoSize)
	...
	const auto badge = XGramUserpic::OnlineBadgeRect(photoSize, size, stroke);
...
void Row::paintUserpic(
	...
	const auto stroke = st::dialogsOnlineBadgeStroke;
	const auto badge = XGramUserpic::OnlineBadgeRect(
		context.st->photoSize,
		size,
		stroke);
	p.setOpacity(
