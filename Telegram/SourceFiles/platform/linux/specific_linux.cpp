...
bool GenerateDesktopFile(
	...
	const auto sourceFile = u":/misc/one.xgram.desktop.desktop"_q;
	...
	if (!Core::UpdaterDisabled()) {
		...
		if (!Core::Launcher::Instance().customWorkingDir()) {
			QFile::remove(u"%1xgram.desktop._%2.desktop"_q.arg(
				targetPath,
				md5Hash));
		...
		QFile::remove(u"%1xgram.desktop.%2.desktop"_q.arg(
			targetPath,
			md5Hash));
	}
...
void start() {
	QGuiApplication::setDesktopFileName([&] {
		...
		if (!Core::UpdaterDisabled()) {
			return u"one.xgram.desktop._%1"_q.arg(
				Core::Launcher::Instance().instanceHash().constData());
		}

		return u"one.xgram.desktop"_q;
	}());
