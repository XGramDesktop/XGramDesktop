...
GhostModeAccountSettings *Manager::resolveGhostSettings() const {
	...
	return session ? &XGramSettings::ghost(session) : nullptr;
}
...
void Manager::retranslate() {
	...
	if (_ghostModeMenu) {
		_ghostModeMenu->setTitle(tr::xgram_CategoryGhostMode(tr::now));
	}
	if (_ghostMode) {
		if (const auto ghost = resolveGhostSettings()) {
			_ghostMode->setText(ghost->isGhostModeActive()
				? tr::xgram_DisableGhostMode(tr::now)
				: tr::xgram_EnableGhostMode(tr::now));
		} else {
			_ghostMode->setText(tr::xgram_EnableGhostMode(tr::now));
		}
	}
	if (_readOnInteract) {
		_readOnInteract->setText(tr::xgram_MarkReadAfterAction(tr::now));
	}
	if (_scheduleMessages) {
		_scheduleMessages->setText(tr::xgram_UseScheduledMessages(tr::now));
	}
}
...
void Manager::buildMenu() {
	buildAppleMenu(_menuBar->addMenu(u"XGram"_q));
