...
[[nodiscard]] QImage ImageIconWithCounter(
	...
	static auto lastUsedIcon = XGramAssets::currentAppLogoName();

	if (lastUsedIcon != XGramAssets::currentAppLogoName()) {
		lastUsedIcon = XGramAssets::currentAppLogoName();
	...
	QPainter p(&result);
	PainterHighQualityEnabler hq(p); // XGram: fix for lq icons
