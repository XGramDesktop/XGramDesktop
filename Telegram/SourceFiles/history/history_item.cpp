...
std::unique_ptr<Data::Media> HistoryItem::CreateMedia(
	...
	return media.match([&](const MTPDmessageMediaContact &media) -> Result {
		...
		if (false) {  // XGram: show expiring messages
			LOG(("App Error: "
			...
			if (checked == MediaCheckResult::HasUnsupportedTimeToLive) {
				media->match(
					[&](const MTPDmessageMediaPhoto &media)
					{
						...
						setXGramHint(formatTTL(time, false));
					...
					[&](const MTPDmessageMediaDocument &media)
					{
						...
						const auto time = media.vttl_seconds()->v;
						setXGramHint(formatTTL(time, true));
						_unsupportedTTL = time;
					},
					[](const auto &) {});
			}
...
bool HistoryItem::isXGramNoForwards() const {
...
void HistoryItem::clearMediaAsExpired() {
	...
	if (settings.saveDeletedMessages()) {
		XGramMessages::addDeletedMessage(this);
		return;
	}
...
const std::vector<Data::MessageReaction> &HistoryItem::reactions() const {
	...
	return _reactions && !XGramFeatures::MessageShot::ignoreRender(XGramFeatures::MessageShot::RenderPart::Reactions) ? _reactions->list() : kEmpty;
}

std::vector<Data::MessageReaction> HistoryItem::reactionsWithLocal() const {
	if (!_reactions || XGramFeatures::MessageShot::ignoreRender(XGramFeatures::MessageShot::RenderPart::Reactions)) {
		return {};
	}
...
void HistoryItem::setDeleted() {
	...
	if (isService()) {
		const auto &settings = XGramSettings::getInstance();
		setXGramHint(settings.deletedMark());
	} else {
		history()->owner().requestItemViewRefresh(this);
		history()->owner().requestItemResize(this);
	}
...
void HistoryItem::setXGramHint(const QString &hint) {
	try {
		auto msgsigned = Get<HistoryMessageSigned>();
		if (hint.isEmpty()) {
			if (!msgsigned) {
		...
		DEBUG_LOG(("XGram: crash in setting hint"));
	}
}
