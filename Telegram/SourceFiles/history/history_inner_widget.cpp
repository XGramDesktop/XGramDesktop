...
void HistoryInner::showContextMenu(QContextMenuEvent *e, bool showFromTouch) {
	...
	const auto addItemActions = [&](
		...
		XGramUi::AddHistoryAction(_menu, item);
		XGramUi::AddHideMessageAction(_menu, item);
		XGramUi::AddUserMessagesAction(_menu, item);
		XGramUi::AddRepeatMessageAction(_menu, item, HistoryView::Context::History);
		XGramUi::AddMessageDetailsAction(_menu, item);
	};
	...
	const auto addSelectMessageAction = [&](not_null<HistoryItem*> item) {
		...
		XGramUi::AddReadUntilAction(_menu, item);
		XGramUi::AddBurnAction(_menu, item);
	};
	...
	if (lnkPhoto || lnkDocument) {
		...
		if (isUponSelected > 0) {
			...
			XGramUi::AddCreateFilterAction(_menu, _controller, item, selectedText.rich.text);
