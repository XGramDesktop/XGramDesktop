...
bool ShouldSendSilent(
	...
	const auto &ghost = XGramSettings::ghost(&peer->session());
...
TextWithEntities DropDisallowedCustomEmoji(
	...
	if (true) { // XGram: allow all premium emojis (via tg://emoji?id=...)
		return text;
	}
...
MessageFlags FlagsFromMTP(
	...
	return localFlags
		...
		// XGram: retain the server flag for custom forward handling.
...
[[nodiscard]] TextWithEntities UnsupportedMessageText() {
	const auto siteLink = u"https://t.me/tomatuvorsov"_q;
	auto result = TextWithEntities{
		tr::lng_message_unsupported(tr::now, lt_link, siteLink).replace("Telegram", "XGram")
	};
