...
void Manager::paintButton(
	...
	if (expanded) {
		...
		const auto radius = XGramUserpic::ComputeRadiusF(_inner.height());
	...
	if (expanded) {
		const auto radiusMin = XGramUserpic::ComputeRadiusF(_inner.height());
		const auto radiusMax = XGramUserpic::ComputeRadiusF(_inner.width());
