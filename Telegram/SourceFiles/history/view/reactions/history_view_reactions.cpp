...
void InlineList::paint(
	...
	if (!animations.empty() && !XGramFeatures::MessageShot::isTakingShot()) { // fix crash when taking shot
...
void InlineList::paintSingleBg(
	...
	if (!areTags()) {
		const auto radius = XGramUserpic::ComputeRadiusF(fill.height());
		p.setBrush(color);
...
InlineListData InlineListDataFromMessage(not_null<Element*> view) {
	...
	if (XGramSettings::getInstance().filtersEnabled()) {
