...
[[nodiscard]] int ComputeRightBadgeWidth(
	...
	if (badge->role == BadgeRole::User) {
		const auto tagWidth = (badge->channel
				&& XGramSettings::getInstance().replaceBottomInfoWithIcons())
...
void Message::refreshRightBadge() {
	...
	if (hasOutLayout() && !XGramFeatures::MessageShot::isTakingShot()) {
		if (Has<RightBadge>()) {
			RemoveComponents(RightBadge::Bit());
		}
		return;
	}
	if (XGramFeatures::MessageShot::ignoreRender(
			XGramFeatures::MessageShot::RenderPart::HeaderDecorations)) {
		if (Has<RightBadge>()) {
			RemoveComponents(RightBadge::Bit());
		}
		return;
...
int Message::marginTop() const {
	...
	if (const auto service = Get<ServicePreMessage>()) {
		if (!service->below && !XGramFeatures::MessageShot::isTakingShot()) {
...
void Message::draw(Painter &p, const PaintContext &context) const {
	...
	const auto displayInfo = needInfoDisplay() && !XGramFeatures::MessageShot::ignoreRender(XGramFeatures::MessageShot::RenderPart::Date);
	const auto reactionsInBubble = _reactions && embedReactionsInBubble();
...
BottomRippleMask Message::bottomRippleMask(int buttonHeight) const {
	...
	if (XGramSettings::getInstance().removeMessageTail()) {
