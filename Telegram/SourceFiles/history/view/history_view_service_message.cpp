...
[[nodiscard]] ServiceTimeMetrics ComputeServiceTimeMetrics(const QString &text) {
	...
	const auto &font = st::xgramServiceTimeBadgeFont;
	result.width = font->width(text)
		+ 2 * st::xgramServiceTimeBadgeHorizontalPadding;
	result.height = font->height + 2 * st::xgramServiceTimeBadgeVerticalPadding;
	result.additionalWidth = std::max(
		st::xgramServiceTimeBadgeGap
			+ result.width
			+ st::xgramServiceTimeBadgeTrailingInset
...
void PaintServiceTime(
	...
	p.setFont(st::xgramServiceTimeBadgeFont);
	p.drawText(
		rect.x() + st::xgramServiceTimeBadgeHorizontalPadding,
		rect.y()
			+ st::xgramServiceTimeBadgeVerticalPadding
			+ st::xgramServiceTimeBadgeFont->ascent,
		text);
}
...
void Service::draw(Painter &p, const PaintContext &context) const {
	...
	if (!onlyMedia) {
		...
		const auto lastLineWidthAdd = (lineWidths.size() > 1
			...
			? st::xgramServiceTimeBadgeLastLineWidthAdd
			: 0;
		...
		if (!timeText.isEmpty() && !lineWidths.empty()) {
			...
			const auto lastTextWidth = qMax(
				...
				0);
			const auto gap = lastTextWidth ? st::xgramServiceTimeBadgeGap : 0;
...
, _text(st::msgMinWidth) {
	if (NeedAboutGroup(_history)) {
		...
		&& XGramSettings::getInstance().disableGreetingSticker()) {
