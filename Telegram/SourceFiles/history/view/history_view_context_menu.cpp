...
void AddMessageActions(
	...
	if (request.item) {
		...
		XGramUi::AddHistoryAction(menu, request.item);
		XGramUi::AddHideMessageAction(menu, request.item);
		XGramUi::AddUserMessagesAction(menu, request.item);
		XGramUi::AddRepeatMessageAction(menu, request.item, context);
		XGramUi::AddMessageDetailsAction(menu, request.item);
	}
	...
	if (request.item) {
		XGramUi::AddReadUntilAction(menu, request.item);
		XGramUi::AddBurnAction(menu, request.item);
	}
}
...
void AddWhoReactedAction(
	...
	if (!XGramUi::needToShowItem(settings.showViewsPanelInContextMenu())) {
		return;
	}
...
void AddSelectRestrictionAction(
		...
		bool addIcon) {
	const auto peer = item->history()->peer;
	if ((!peer->isXGramNoForwards() && !XGramForward::isXGramForwardNeeded(item))
		|| item->isSponsored()) {
		return;
	}
	...
	auto button = base::make_unique_q<Ui::Menu::MultilineAction>(
		...
		((addIcon && !user)
			...
			: st::historyHasCustomEmojiPosition),
		tr::xgram_UnforwardableContextMenuText(
			tr::now,
			tr::rich),
...
bool ItemHasTtl(HistoryItem *item) {
	return false; // XGram: allow downloading files with ttl
}
