...
void BottomInfo::paint(
	...
	const auto showMessageSeconds
		= XGramSettings::getInstance().showMessageSeconds();
	if (_showMessageSeconds != showMessageSeconds) {
		// Bottom info text is cached. Rebuild it when the preference changes,
		// otherwise repainting an already open chat keeps the old time format.
		const auto currentWidth = width();
		const auto that = const_cast<BottomInfo*>(this);
		that->layout();
		if (currentWidth > 0) {
			that->resizeGetHeight(std::min(that->maxWidth(), currentWidth));
		}
	}
...
void BottomInfo::layoutDateText() {
	_showMessageSeconds = XGramSettings::getInstance().showMessageSeconds();
	...
	const auto deleted = bool(_data.flags & Data::Flag::XGramDeleted);
	const auto burnt = bool(_data.flags & Data::Flag::XGramBurnt);
	const auto editedFlag = bool(_data.flags & Data::Flag::Edited);
	const auto useStatusIcons = settings.replaceBottomInfoWithIcons();
	const auto statusIconsWidth = !useStatusIcons
		? 0
		: (deleted ? st::deletedIcon.icon.width() + 2 : 0)
			+ (burnt ? st::burntIcon.icon.width() + 2 : 0)
			+ ((editedFlag && !editedPrimary)
				? st::editedIcon.icon.width() + 2
				: 0);
	const auto deletedText = (deleted && !useStatusIcons)
		? (settings.deletedMark() + ' ')
		: QString();
	const auto burntText = (burnt && !useStatusIcons)
		? QString::fromUtf8("🔥 ")
		: QString();
	const auto edited = editedPrimary
		? QString()
		: editedFlag
		? (useStatusIcons ? QString() : settings.editedMark() + ' ')
	...
	const auto afterAuthorWidth = st::msgDateFont->width(afterAuthor)
		+ statusIconsWidth
		+ st::msgDateFont->width(deletedText + burntText);
	...
	if (useStatusIcons) {
		...
		if (editedFlag && !editedPrimary) {
			marked.append(Ui::Text::IconEmoji(&st::editedIcon)).append(' ');
		}
	}
	marked.append(deletedText).append(burntText);
	marked.append(full);
...
BottomInfo::Data BottomInfoDataFromMessage(not_null<Message*> message) {
	...
	if (item->isDeleted()) {
		result.flags |= Flag::XGramDeleted;
	}
	if (item->isBurnt()) {
		result.flags |= Flag::XGramBurnt;
	}
