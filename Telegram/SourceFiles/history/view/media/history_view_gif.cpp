...
void Gif::draw(Painter &p, const PaintContext &context) const {
	...
	if (paintInCenter && !XGramFeatures::MessageShot::isTakingShot()) {
	...
	if (displayMute && !XGramFeatures::MessageShot::isTakingShot()) {
	...
	if (!unwrapped && !skipDrawingSurrounding && !XGramFeatures::MessageShot::ignoreRender(XGramFeatures::MessageShot::RenderPart::Date)) {
		...
		if (isRound && !XGramFeatures::MessageShot::ignoreRender(XGramFeatures::MessageShot::RenderPart::Date)) {
	...
	if (!inWebPage && !skipDrawingSurrounding && !XGramFeatures::MessageShot::ignoreRender(XGramFeatures::MessageShot::RenderPart::Date)) {
...
void Gif::drawCornerStatus(
	...
	const auto cornerMute = _streamed && _data->isVideoFile() && !cornerDownload && !XGramFeatures::MessageShot::isTakingShot();
	...
	if (cornerDownload && !XGramFeatures::MessageShot::isTakingShot()) {
...
bool Gif::needCornerStatusDisplay() const {
	if (XGramFeatures::MessageShot::ignoreRender(XGramFeatures::MessageShot::RenderPart::Date)) {
		return false;
	}
...
void Gif::ensureTranscribeButton() const {
	if (XGramFeatures::MessageShot::isTakingShot()) {
		_transcribe = nullptr;
		return;
	}
