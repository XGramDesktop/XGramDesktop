...
, _muteUnmute(
	...
	st::historyComposeButton)
, _discuss(this,
	tr::xgram_ChannelBottomButtonDiscuss(tr::now).toUpper(),
	st::historyComposeButton)
...
, _topShadow(this) {
	...
	rpl::merge(
		XGramSettings::getInstance().showAttachButtonInMessageFieldChanges() | rpl::to_empty,
		XGramSettings::getInstance().showCommandsButtonInMessageFieldChanges() | rpl::to_empty,
		XGramSettings::getInstance().showEmojiButtonInMessageFieldChanges() | rpl::to_empty,
		XGramSettings::getInstance().showMicrophoneButtonInMessageFieldChanges() | rpl::to_empty,
		XGramSettings::getInstance().showAutoDeleteButtonInMessageFieldChanges() | rpl::to_empty,
		XGramSettings::getInstance().showGiftButtonInMessageFieldChanges() | rpl::to_empty,
		XGramSettings::getInstance().showAiEditorButtonInMessageFieldChanges() | rpl::to_empty,
		XGramSettings::getInstance().showAttachPopupChanges() | rpl::to_empty,
		XGramSettings::getInstance().showEmojiPopupChanges() | rpl::to_empty,
		XGramSettings::getInstance().channelBottomButtonChanges() | rpl::to_empty,
	...
	XGramSettings::getInstance().translationProviderChanges(
	...
	session().api().sendActions(
		...
		if (action.replaceMediaOf) {
			...
			const auto &ghost = XGramSettings::ghost(&controller->session());
...
void HistoryWidget::refreshGiftToChannelShown() {
	...
	// XGram: hide gift button almost everywhere
...
void HistoryWidget::messagesReceived(
	...
	// Expects(_history != nullptr);
	if (!_history || !_peer) {
		return; // XGram: fix crash when using `saveDeletedMessages`
	}
...
void HistoryWidget::updateSendRestriction() {
	...
	if (XGramForward::isForwarding(_peer->id)) {
		_sendRestriction = XGramForwardWriteRestriction(this, _peer->id, session());
	} else if (!restriction) {
...
void HistoryWidget::messageShotSelected() {
	...
	XGramFeatures::MessageShot::Wrapper(
		_list.data(),
		[=] { clearSelected(); });
}
