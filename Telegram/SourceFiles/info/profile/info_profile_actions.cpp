...
void AddRegistrationOrCreationButton(
	...
	button->setAccessibleName(tr::lng_new_contact_registration(tr::now));
	const auto rightSkip = st::infoProfileLabeledButtonQrRightSkip;
	fitLabelToButton(button, idInfo.text, rightSkip);
	fitLabelToButton(button, idInfo.subtext, rightSkip);
	button->setClickedCallback([=] {
		getRegistrationDate(peer, [=](const TextWithEntities &result) {
			if (result.empty()) {
				controller->showToast(tr::xgram_RegistrationDateUnavailable(tr::now));
				return;
			}
			const auto parent = button->window();
			...
			const auto countPosition = [=](QSize size) {
				...
				const auto right = parent->width() - st::normalFont->spacew;
				return QPoint(
			...
			base::install_event_filter(tooltip, qApp, [weak](not_null<QEvent*> e) {
				if (e->type() == QEvent::MouseButtonPress && weak) {
...
Section DetailsFiller::makeInfo() {
	...
	if (!_topic) {
		auto idLine = addInfoLineGeneric(
			rpl::single(TextWithEntities{ getPeerDC(_peer) }),
			IDValue(_peer));
		const auto idText = idLine.text;
		idText->setSelectable(true);
		idText->setDoubleClickSelectsParagraph(true);
		idText->setContextCopyText(tr::xgram_ContextCopyID(tr::now));
		IDValue(_peer) | rpl::on_next([=](const TextWithEntities &) {
			const auto id = IDString(_peer);
			idText->setLink(
				kPeerIdLinkIndex,
				std::make_shared<DraggableUrlClickHandler>(
					u"internal:~peer_id~:copy:"_q + id,
		...
		AddRegistrationOrCreationButton(
			controller,
			_peer,
			idLine,
			fitLabelToButton);
