	...
	rpl::combine(
		...
		if (item) {
			if (const auto document = item->media()
					? item->media()->document()
					: nullptr) {
				auto musicButton = divider->entity()->add(object_ptr<Ui::SlideWrap<Profile::XGramMusicButton>>(
					divider->entity(),
					object_ptr<Profile::XGramMusicButton>(
						divider->entity(),
				...
				}) | rpl::on_next([=]
										  {
											  ...
											  contextMenu->addAction(
												  settings.adaptiveCoverColor()
													  ? tr::xgram_DisableColorfulCover(tr::now)
													  : tr::xgram_EnableColorfulCover(tr::now),
												  [=]
												  {
													  XGramSettings::getInstance().setAdaptiveCoverColor(!XGramSettings::getInstance().adaptiveCoverColor());
