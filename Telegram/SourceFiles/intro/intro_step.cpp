...
void Step::paintCover(QPainter &p, int top) {
	...
	const auto xgramGramIcon = Ui::PixmapFromImage(XGramAssets::currentAppLogo());
	QIcon(xgramGramIcon).paint(&p, QRect(width() / 2 - xgramGramIcon.width() / 2, planeTop - 16, xgramGramIcon.width(), st::introCoverIcon.height()));
}
