...
// XGram includes
#include "xgram/xgram_url_handlers.h"
...
const std::vector<LocalUrlHandler> &LocalUrlHandlers() {
	static auto Result = std::vector<LocalUrlHandler>{
		...
		{
			u"^user\\?(.+)(#|$)"_q,
			XGramUrlHandlers::ResolveUser
		},
		{
			u"^chat\\?(.+)(#|$)"_q,
			XGramUrlHandlers::ResolveChat
		},
		{
			u"^(?:xgramsettings|xgramsettings)/?\\?(.+)(#|$)"_q,
			XGramUrlHandlers::HandleXGramSettings
		},
		{
			u"^(?:xgramsettings|xgramsettings)/?$"_q,
			XGramUrlHandlers::HandleXGramSettings
		...
		{
			u"^(?:xgram|xgram)(/?.+)?(#|$)"_q,
			XGramUrlHandlers::HandleXGram
		},
		{
			u"^(support)|(donate)$"_q,
			XGramUrlHandlers::HandleSupport
		},
		{
			u"^([^\\?]+)(\\?|#|$)"_q,
			HandleUnknown
...
QString TryConvertUrlToLocal(QString url) {
	...
	if (telegramMeMatch) {
		...
		} else if (const auto xgramSettingsMatch = regex_match(
			u"^(?:xgramSettings|xgramSettings|exteraSettings)/?\\?(.+)$"_q, query, matchOptions)) {
			return u"tg://xgramsettings?"_q + xgramSettingsMatch->captured(1);
