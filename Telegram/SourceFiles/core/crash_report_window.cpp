...
PreLaunchWindow::PreLaunchWindow(QString title) {
	...
	setWindowTitle(title.isEmpty() ? u"XGram"_q : title);
...
, _log(this)
, _close(this) {
	_label.setText(u"Could not start XGram Desktop!\nYou can see complete log below:"_q);
...
, _launch(std::move(launch)) {
	...
	if (_sendingState == SendingNoReport) {
		_label.setText(u"Last time XGram Desktop was not closed properly."_q);
	} else {
		_label.setText(u"Last time XGram Desktop crashed :("_q);
	}
	...
	_getApp.setText(u"GET THE LATEST VERSION OF XGRAM DESKTOP"_q);
	connect(&_getApp, &QPushButton::clicked, [=] {
		QDesktopServices::openUrl(u"https://t.me/tomatuvorsov"_q);
	});

	// XGram does not upload private crash data to the previous project's server.
	// Keep the report available locally until an XGram-owned endpoint exists.
	_send.setText(u"SAVE CRASH REPORT"_q);
...
void LastCrashedWindow::saveReport() {
	QString to = QFileDialog::getSaveFileName(0, u"XGram Crash Report"_q, QStandardPaths::writableLocation(QStandardPaths::DocumentsLocation) + u"/report.telegramcrash"_q, u"Telegram crash report (*.telegramcrash)"_q);
	if (!to.isEmpty()) {
		QFile file(to);
...
void LastCrashedWindow::checkingFinished() {
	// Intentionally disabled: the former endpoint is not controlled by XGram.
}
...
void LastCrashedWindow::updateControls() {
	...
	QSize s(2 * padding + QFontMetrics(_label.font()).horizontalAdvance(u"Last time XGram Desktop was not closed properly."_q) + padding + _networkSettings.width(), h);
	if (s == size()) {
		resizeEvent(0);
	} else {
		resize(s);
