...
TextWithEntities ChangeSummaryText(const ApplyChanges &changes) {
	...
	if (!changes.newFilters.empty()) {
		AppendChangeSummary(
			result,
			tr::xgram_FiltersSheetNewFilters(
				tr::now,
				lt_count,
	...
	if (!changes.removeFiltersById.empty()) {
		AppendChangeSummary(
			result,
			tr::xgram_FiltersSheetRemovedFilters(
				tr::now,
				lt_count,
	...
	if (!changes.filtersOverrides.empty()) {
		AppendChangeSummary(
			result,
			tr::xgram_FiltersSheetUpdatedFilters(
				tr::now,
				lt_count,
	...
	if (!changes.newExclusions.empty()) {
		AppendChangeSummary(
			result,
			tr::xgram_FiltersSheetNewExclusions(
				tr::now,
				lt_count,
	...
	if (!changes.removeExclusions.empty()) {
		AppendChangeSummary(
			result,
			tr::xgram_FiltersSheetRemovedExclusions(
				tr::now,
				lt_count,
	...
	if (!changes.peersToBeResolved.empty()) {
		AppendChangeSummary(
			result,
			tr::xgram_FiltersSheetDialogsToResolve(
				tr::now,
				lt_count,
...
void FilterUtils::importFromLink(const QString &link) {
	if (link.isEmpty()) {
		Ui::Toast::Show(tr::xgram_FiltersToastFailFetch(tr::now));
		return;
	}
	...
	connect(
		...
		[=]
		{
			...
			if (jsonString.isNull()) {
				...
				Ui::Toast::Show(tr::xgram_FiltersToastFailImport(tr::now));
...
void FilterUtils::publishFilters() {
	...
	titlePart.setBody("XGram Filters");
	...
	connect(
		...
		[=]
		{
			...
			if (error == QNetworkReply::NoError && location.isValid()) {
				...
				Ui::Toast::Show(tr::xgram_FiltersToastFailPublish(tr::now));
			}
			reply->deleteLater();
		});
}
...
void FilterUtils::importFromJson(const QByteArray &json) {
	...
	if (!HasChanges(changes)) {
		Ui::Toast::Show(tr::xgram_FiltersToastFailNoChanges(tr::now));
	...
	auto box = Ui::MakeConfirmBox({
		...
		.confirmed = [=](Fn<void()> close) {
			...
			try {
				...
				Ui::Toast::Show(tr::xgram_FiltersToastSuccess(tr::now));
			} catch (...) {
				LOG(("FilterUtils: Failed to apply import changes"));
				Ui::Toast::Show(tr::xgram_FiltersToastFailImport(tr::now));
			}
		},
		.confirmText = tr::xgram_FiltersMenuImport(),
		.title = tr::xgram_FiltersSheetTitle(),
	});
...
QString FilterUtils::exportFilters() {
	...
	const auto filters = XGramDatabase::getAllRegexFilters();
	...
	const auto excl = XGramDatabase::getAllFiltersExclusions();
...
ApplyChanges FilterUtils::prepareChanges(const QJsonObject &root) {
	...
	const auto existingFilters = XGramDatabase::getAllRegexFilters();
	const auto existingExclusions = XGramDatabase::getAllFiltersExclusions();
...
void FilterUtils::applyChanges(const ApplyChanges &changes) {
	if (!changes.newFilters.empty()) {
		for (const auto &filter : changes.newFilters) {
			XGramDatabase::addRegexFilter(filter);
		}
	}

	if (!changes.removeFiltersById.empty()) {
		for (const auto &id : changes.removeFiltersById) {
			XGramDatabase::deleteExclusionsByFilterId(id);
			XGramDatabase::deleteFilter(id);
		}
	}

	if (!changes.filtersOverrides.empty()) {
		for (const auto &filter : changes.filtersOverrides) {
			XGramDatabase::updateRegexFilter(filter);
		}
	}

	if (!changes.newExclusions.empty()) {
		for (const auto &exclusion : changes.newExclusions) {
			XGramDatabase::addRegexExclusion(exclusion);
		}
	}

	if (!changes.removeExclusions.empty()) {
		for (const auto &exclusion : changes.removeExclusions) {
			XGramDatabase::deleteExclusion(exclusion.dialogId, exclusion.filterId);
		}
	}
