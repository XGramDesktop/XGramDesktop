...
#include "xgram/features/translator/xgram_translate_provider.h"

#include "api/api_text_entities.h"
#include "xgram/features/translator/xgram_translator.h"
...
class XGramTranslateProvider final
	: public Ui::TranslateProvider
	, public base::has_weak_ptr {
public:
	XGramTranslateProvider(
		not_null<Main::Session*> session,
		TranslationProvider provider)
	: _session(session)
	, _provider(provider) {
		if (!XGram::Translator::TranslateManager::currentInstance()) {
	...
	void requestBatch(
		...
		const auto manager = XGram::Translator::TranslateManager::currentInstance();
...
std::unique_ptr<TranslateProvider> CreateXGramTranslateProvider(
		not_null<Main::Session*> session,
		TranslationProvider provider) {
	return std::make_unique<XGramTranslateProvider>(session, provider);
}
