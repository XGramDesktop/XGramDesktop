...
#include "apiwrap.h"
#include "lang_auto.h"
#include "xgram/features/forward/xgram_sync.h"
...
std::pair<QString, QString> stateName(const PeerId &id) {
	...
	QString messagesString = tr::xgram_XGramForwardStatusSentCount(tr::now,
															   lt_count1,
	...
	QString chunkString = tr::xgram_XGramForwardStatusChunkCount(tr::now,
															 lt_count1,
	...
	if (state->state == ForwardState::State::Preparing) {
		status = tr::xgram_XGramForwardStatusPreparing(tr::now);
	} else if (state->state == ForwardState::State::Downloading) {
		return std::make_pair(tr::xgram_XGramForwardStatusLoadingMedia(tr::now), "");
	} else if (state->state == ForwardState::State::Sending) {
		status = tr::xgram_XGramForwardStatusForwarding(tr::now);
	} else {
		// ForwardState::State::Finished
		status = tr::xgram_XGramForwardStatusFinished(tr::now);
	}
...
static Ui::PreparedList prepareMedia(not_null<Main::Session*> session,
	...
	const auto prepare = [&](not_null<Data::Media*> media)
	{
		...
		auto prepared = Ui::PreparedFile(XGramSync::filePath(session, media));
...
void sendMedia(
	...
	if (const auto document = primaryMedia->document(); document && document->sticker()) {
		XGramSync::sendStickerSync(session, std::move(message), document);
		return;
	}
	...
	if (mediaType == SendMediaType::Round || mediaType == SendMediaType::Audio) {
		...
		if (!failed && data.size()) {
			file.close();
			XGramSync::sendVoiceSync(session,
								   data,
...
bool isXGramForwardNeeded(const std::vector<not_null<HistoryItem*>> &items) {
	const auto needXGramForward = [&](const auto &item)
	{
		return isXGramForwardNeeded(item);
	};
	return std::ranges::any_of(items, needXGramForward);
}

bool isXGramForwardNeeded(not_null<HistoryItem*> item) {
	if (item->isDeleted() || item->isXGramNoForwards() || item->unsupportedTTL() || (item->media() && item->media()->ttlSeconds())) {
		return true;
	}
	return false;
}

bool isFullXGramForwardNeeded(not_null<HistoryItem*> item) {
	return item->from()->isXGramNoForwards() || item->history()->peer->isXGramNoForwards();
}

struct ForwardChunk
{
	bool isXGramForwardNeeded;
...
void intelligentForward(
	...
	auto currentChunk = ForwardChunk({
		.isXGramForwardNeeded = isXGramForwardNeeded(items[0]),
		.items = currentArray
	});

	for (const auto &item : items) {
		if (isXGramForwardNeeded(item) != currentChunk.isXGramForwardNeeded) {
			...
			currentChunk = ForwardChunk({
				.isXGramForwardNeeded = isXGramForwardNeeded(item),
				.items = currentArray
			});
		}
	...
	for (const auto &chunk : chunks) {
		if (chunk.isXGramForwardNeeded) {
			...
			XGramSync::forwardMessagesSync(session, chunk.items, action, draft.options);
...
void forwardMessages(
	...
	if (!toBeDownloaded.empty()) {
		...
		XGramSync::loadDocuments(session, toBeDownloaded);
	}
	...
	for (int i = 0; i < items.size(); i++) {
		...
		if (!mediaDownloadable(item->media())) {
			XGramSync::sendMessageSync(session, std::move(message));
...
} // namespace XGramFeatures::XGramForward
