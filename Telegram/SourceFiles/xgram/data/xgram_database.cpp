...
#include "xgram/libs/sqlite/sqlite_orm.h"
	...
	for (int v = currentVersion + 1; v <= kLatestVersion; ++v) {
		if (migrations.contains(v)) {
			try {
				...
				XGramDatabase::moveCurrentDatabase();
...
namespace XGramDatabase {

namespace {

void migrateLegacyDatabase() {
	const auto migrate = [](const QString &suffix) {
		const auto legacy = u"./tdata/xgramdata.db"_q + suffix;
		const auto current = u"./tdata/xgram-data.db"_q + suffix;
...
void moveCurrentDatabase() {
	...
	if (QFile::exists("./tdata/xgram-data.db")) {
		QFile::rename("./tdata/xgram-data.db", QString("./tdata/xgram-data_%1.db").arg(time));
	}

	if (QFile::exists("./tdata/xgram-data.db-shm")) {
		QFile::rename("./tdata/xgram-data.db-shm", QString("./tdata/xgram-data_%1.db-shm").arg(time));
	}

	if (QFile::exists("./tdata/xgram-data.db-wal")) {
		QFile::rename("./tdata/xgram-data.db-wal", QString("./tdata/xgram-data_%1.db-wal").arg(time));
	}
}
	...
	try {
		...
		storage.remove_all<DeletedMessage>(
			where(
				...
				column<DeletedMessage>(&DeletedMessage::topicId) == message.topicId and
...
void clearAllDeletedMessages(ID userId) {
	try {
		storage.remove_all<DeletedMessage>(
