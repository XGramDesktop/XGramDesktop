...
bool ResolveUser(
	...
	searchUserById(
		...
		[=](const QString &title, PeerData *data)
		{
			...
			Ui::show(Ui::MakeInformBox(tr::xgram_UserNotFoundMessage()));
		}
	);
...
struct ResolvedSetting {
	QString controlId;
	::Settings::Type section = ::Settings::XGramMain::Id();
};
...
bool HandleXGramSettings(
	...
	if (settingName.isEmpty()) {
		controller->showSettings(::Settings::XGramMain::Id());
	} else {
		const auto resolved = ResolveSetting(
			u"xgram/"_q + settingName,
			&controller->session());
