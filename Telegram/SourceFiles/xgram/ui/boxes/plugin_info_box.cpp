...
#include "xgram/ui/boxes/plugin_info_box.h"
...
void FillPluginInfoBox(
...

	{
		...
		const auto versionPrefix = tr::xgram_PluginVersion(tr::now)
			+ u" "_q
			+ metadata.version;
		...
		if (metadata.author.isEmpty()) {
			versionLabel->setText(
				versionPrefix
				+ u" \u2022 "_q
				+ tr::xgram_PluginNoAuthor(tr::now));
		} else {
...

	{
		...
		auto descText = hasDescription
			...
			: TextWithEntities{tr::xgram_PluginNoDescription(tr::now)};
		if (hasDescription) {
