...
void Content::setupContent(
	...
	const auto current = XGramUiSettings::getMonoFont();
...
XGramUi::FontSelectorBox::FontSelectorBox(QWidget *, Window::SessionController *controller, Fn<void(QString font)> hook)
	: _controller(controller), _hook(hook) {
}

void XGramUi::FontSelectorBox::prepare() {
	...
	addLeftButton(tr::xgram_BoxActionReset(),
				  [=]
				  {
					  _hook(qs(""));
	...
	setTitle(tr::xgram_CustomizeFontTitle());
	...
	const auto fonts = XGramUi::PrepareFonts();
	const auto inner = setInnerWidget(
		object_ptr<XGramUi::Content>(this, fonts),
		st::boxScroll,
		topContainer->height());
...
void XGramUi::FontSelectorBox::setupTop(not_null<Ui::VerticalLayout*> container) {
	if (!_controller) {
		return;
	}
}

void XGramUi::FontSelectorBox::keyPressEvent(QKeyEvent *e) {
...
int XGramUi::FontSelectorBox::rowsInPage() const {
	return std::max(height() / XGramUi::Rows::DefaultRowHeight(), 1);
}

void XGramUi::FontSelectorBox::setInnerFocus() {
	_setInnerFocus();
}

base::binary_guard
XGramUi::FontSelectorBox::Show(Window::SessionController *controller, const Fn<void(QString font)> hook) {
