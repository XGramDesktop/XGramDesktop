...
#include "lang_auto.h"
#include "xgram/features/message_shot/message_shot_theme_state.h"
...
void ThemeSelectorBox::setupContent() {
	...
	setTitle(tr::xgram_MessageShotThemeSelectTitle());
	...
	XGramFeatures::MessageShot::themeChosen(
	) | rpl::on_next(
	...
	XGramFeatures::MessageShot::paletteChosen(
	) | rpl::on_next([=](const auto &palette)
							 {
								 ...
								 const auto name = (type != Window::Theme::EmbeddedType(-1))
									 ? XGramFeatures::MessageShot::embeddedThemeDisplayName(type)
									 : tr::xgram_MessageShotThemeDefault(tr::now);
								 _themeNames.fire(QString(name));
								 _selectedPalette = palette;
							 },
							 lifetime());

	addButton(tr::xgram_MessageShotThemeApply(),
			  [=]
			  {
