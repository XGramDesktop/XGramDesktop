...
void FillImportFiltersBox(not_null<Ui::GenericBox*> box, bool import) {
	...
	box->setTitle(import ? tr::xgram_FiltersMenuImport() : tr::xgram_FiltersMenuExport());
	...
	addOption(false, import ? tr::xgram_FiltersImportClipboard(tr::now) : tr::xgram_FiltersExportClipboard(tr::now));
	addOption(true, import ? tr::xgram_FiltersImportURL(tr::now) : tr::xgram_FiltersExportURL(tr::now));
	...
	const auto actionButton = box->addButton(
		import ? tr::xgram_FiltersMenuImport() : tr::xgram_FiltersMenuExport(),
		[=]
		{
