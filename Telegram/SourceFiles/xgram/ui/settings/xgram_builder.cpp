...
namespace Settings::XGramBuilder {
namespace {
...
XGramSectionBuilder::XGramSectionBuilder(Builder::SectionBuilder &builder)
: _builder(builder) {
}

Builder::SectionBuilder &XGramSectionBuilder::base() {
	return _builder;
}

Ui::SettingsButton *XGramSectionBuilder::addSettingToggle(
		SettingToggleArgs &&args) {
	auto getter = [g = args.getter] {
		return (XGramSettings::getInstance().*g)();
	};
	auto setter = [s = args.setter](bool v) {
		(XGramSettings::getInstance().*s)(v);
	};
	return addToggle({
		.id = std::move(args.id),
...
Ui::SettingsButton *XGramSectionBuilder::addToggle(ToggleArgs &&args) {
...
Fn<void()> XGramSectionBuilder::addCollapsibleToggle(
...
void XGramSectionBuilder::addChooseButton(ChooseButtonArgs &&args) {
...
void XGramSectionBuilder::addSlider(SliderArgs &&args) {
	auto id = std::move(args.id);
	auto altIds = std::move(args.altIds);
	auto keywords = std::move(args.keywords);
	auto resolvedTitle = ResolveTitle(rpl::duplicate(args.title));
...
void XGramSectionBuilder::addBetaBadge(not_null<Ui::SettingsButton*> button) {
	AddBetaBadge(button);
}

void XGramSectionBuilder::addSectionDivider() {
	_builder.addSkip();
	_builder.addDivider();
	_builder.addSkip();
}
