...
void RegexEditBuilder(
	...
	if (filter) {
		box->setTitle(showToast ? tr::xgram_RegexFiltersAdd() : tr::xgram_RegexFiltersEdit());
		data = *filter;
	} else {
		box->setTitle(tr::xgram_RegexFiltersAdd());
		data.enabled = true;
	...
	const auto caseInsensitive = box->addRow(
		object_ptr<Ui::Checkbox>(
			box,
			tr::xgram_CaseInsensitiveExpression(tr::now),
	...
	const auto reversed = box->addRow(
		object_ptr<Ui::Checkbox>(
			box,
			tr::xgram_ReversedExpression(tr::now),
			data.reversed,
			st::defaultBoxCheckbox),
		st::settingsCheckboxPadding);
	...
	auto saveAndClose = [=, id = data.id]
	{
		...
		crl::async([=]
		{
			XGramDatabase::addRegexFilter(newFilter);
			...
			crl::on_main([=]
			{
				...
				if (showToast) {
					auto config = Ui::Toast::Config{
						.text = tr::xgram_RegexFilterBulletinText(
							tr::now,
							tr::rich),
						.adaptive = true,
					};
					if (dialogId.has_value()) {
						XGram::Ui::ShowToastWithAction(
							std::move(config),
							tr::xgram_RegexFilterBulletinAction(tr::now),
							[=]() mutable {
								newFilter.dialogId = dialogId;

								XGramDatabase::updateRegexFilter(newFilter);
								FiltersCacheController::rebuildCache();
								FiltersCacheController::fireUpdate();
							});
					} else {
