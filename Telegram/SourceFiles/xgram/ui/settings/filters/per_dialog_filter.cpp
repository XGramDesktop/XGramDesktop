...
void PerDialogFiltersListController::prepare() {
	...
	const auto exclusions = XGramDatabase::getAllFiltersExclusions();
	...
	for (const auto &[id, count] : countsByDialogIds) {
		...
		auto status = QString();
		if (count.filters > 0) {
			status += tr::xgram_RegexFiltersAmount(tr::now, lt_count, count.filters);
			if (count.exclusions > 0) {
				status += ", ";
			}
		}
		if (count.exclusions > 0) {
			status += tr::xgram_RegexFiltersExcludedAmount(tr::now, lt_count, count.exclusions);
		}
...
void PerDialogFiltersListController::rowClicked(not_null<PeerListRow*> peer) {
	...
	if (shadowBan) {
		...
		_contextMenu->addAction(
			...
			[=]
			{
				if (XGramSettings::getInstance().isShadowBanned(did)) {
					XGramSettings::getInstance().removeShadowBan(did);
				} else {
					XGramSettings::getInstance().addShadowBan(did);
				}
			},
			&st::menuIconDelete);
	...
	_controller->showSettings(XGramFiltersList::Id());
}
