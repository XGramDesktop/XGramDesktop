...
#include "xgram/xgram_state.h"
#include "xgram/data/messages_storage.h"
#include "xgram/features/filters/filters_controller.h"
#include "xgram/features/forward/xgram_forward.h"
#include "xgram/ui/context_menu/menu_item_subtext.h"
#include "xgram/ui/message_history/history_section.h"
#include "xgram/ui/settings/filters/edit_filter.h"
#include "xgram/ui/settings/filters/settings_filters_list.h"
#include "xgram/utils/qt_key_modifiers_extended.h"
...
Fn<void()> ClearDeletedMessagesHandler(not_null<Window::SessionController*> controller, not_null<PeerData*> peer, ID topicId) {
	return [=] {
		controller->show(Ui::MakeConfirmBox({
			.text = tr::xgram_ClearDeletedMessagesText(tr::now),
			.confirmed = [=](Fn<void()> &&close) {
				...
				XGramMessages::clearDeletedMessages(peer, topicId);
				for (const auto item : items) {
					item->destroy();
				}
				close();
			},
			.confirmText = tr::xgram_ClearDeletedMessagesActionText(tr::now),
...
Fn<void()> DeleteMyMessagesHandler(not_null<Window::SessionController*> controller, not_null<PeerData*> peer) {
	return [=]
	{
		if (!controller->showFrozenError()) {
			controller->show(Ui::MakeConfirmBox({
				.text = tr::xgram_DeleteOwnMessagesConfirmation(tr::now),
				.confirmed =
				[=](Fn<void()> &&close)
				{
...
void AddXGramActions(PeerData *peerData,
							   Data::Thread *thread,
	...
	const auto showPerChatGhostMode = settings.perChatGhostMode()
		&& !peerData->isSelf();
	const auto showGlobalPins = settings.globalPinsEnabled();
	if (!showFilters && !saveDeletedMessages && !showPerChatGhostMode
		&& !showGlobalPins) {
		return;
	}
	...
	if (showGlobalPins) {
		const auto accountId = peerData->session().userId().bare;
		const auto peerId = peerData->id.value;
		const auto pinned = settings.isGloballyPinned(accountId, peerId);
		addCallback(
			pinned
				? tr::xgram_GlobalUnpin(tr::now)
				: tr::xgram_GlobalPin(tr::now),
			[=] {
				XGramSettings::getInstance().setGloballyPinned(
					accountId,
					peerId,
					!pinned);
			},
	...
	if (showPerChatGhostMode) {
		const auto accountId = peerData->session().userId().bare;
		const auto peerId = peerData->id.value;
		const auto active = settings.isGhostModeInChat(accountId, peerId);
		addCallback(
			active
				? tr::xgram_DisableGhostModeInChat(tr::now)
				: tr::xgram_EnableGhostModeInChat(tr::now),
			[=] {
				XGramSettings::getInstance().setGhostModeInChat(
					accountId,
					peerId,
					!active);
			},
	...
	addCallback(Window::PeerMenuCallback::Args{
		...
		.fillSubmenu = [=](not_null<Ui::PopupMenu*> menu) {
			...
			if (showFilters) {
				addAction(
					tr::xgram_ViewFiltersMenuText(tr::now),
					[=]
					{
						...
						sessionController->showSettings(Settings::XGramFiltersList::Id());
					},
			...
			if (filteredToggleShown) {
				addAction(
					*filteredToggleShown
						? tr::xgram_HideFilteredMessagesMenuText(tr::now)
						: tr::xgram_ShowFilteredMessagesMenuText(tr::now),
					[=]
					{
			...
			if (saveDeletedMessages) {
				addAction(
					tr::xgram_ViewDeletedMenuText(tr::now),
					[=]
					{
				...
				addAction({
					.text = tr::xgram_ClearDeletedMenuText(tr::now),
...
void AddJumpToBeginningAction(PeerData *peerData,
	...
	addCallback(
		tr::xgram_JumpToBeginning(tr::now),
		[=]
		{
			if (user) {
...
void AddShadowBanAction(PeerData *peerData,
	...
	const auto shadowBanned = XGramSettings::getInstance().isShadowBanned(realId);
	const auto toggleShadowBan = [=]
	{
		if (shadowBanned) {
			XGramSettings::getInstance().removeShadowBan(realId);
		} else {
			XGramSettings::getInstance().addShadowBan(realId);
		}
	};

	addCallback({
		.text = (shadowBanned
					 ? tr::xgram_FiltersQuickUnshadowBan(tr::now)
					 : tr::xgram_FiltersQuickShadowBan(tr::now)),
...
void AddDeleteOwnMessagesAction(PeerData *peerData,
	...
	addCallback(
		tr::xgram_DeleteOwnMessages(tr::now),
...
void AddHistoryAction(not_null<Ui::PopupMenu*> menu, HistoryItem *item) {
	...
	const auto has = XGramMessages::hasRevisions(item);
	if (!has) {
		return;
	}

	menu->addAction(
		tr::xgram_EditsHistoryMenuText(tr::now),
		[=]
		{
...
void AddHideMessageAction(not_null<Ui::PopupMenu*> menu, HistoryItem *item) {
	...
	menu->addAction(
		tr::xgram_ContextHideMessage(tr::now),
		[=]()
		{
...
void AddUserMessagesAction(not_null<Ui::PopupMenu*> menu, HistoryItem *item) {
	...
	if (item->history()->peer->isChat() || item->history()->peer->isMegagroup()) {
		menu->addAction(
			tr::xgram_UserMessagesMenuText(tr::now),
			[=]
			{
...
void AddMessageDetailsAction(not_null<Ui::PopupMenu*> menu, HistoryItem *item) {
	...
	callback(Window::PeerMenuCallback::Args{
		.text = tr::xgram_MessageDetailsPC(tr::now),
		.handler = nullptr,
		...
		.fillSubmenu = [&](not_null<Ui::PopupMenu*> menu2)
		{
			if (hasAnyPostField) {
				if (!messageViews.isEmpty()) {
					menu2->addAction(Ui::ContextActionWithSubText(
						...
						tr::xgram_MessageDetailsViewsPC(tr::now),
						messageViews
					));
				}
				...
				if (!messageForwards.isEmpty()) {
					menu2->addAction(Ui::ContextActionWithSubText(
						...
						tr::xgram_MessageDetailsSharesPC(tr::now),
						messageForwards
					));
				}
			...
			menu2->addAction(Ui::ContextActionWithSubText(
				menu2->menu(),
				st::menuIconSchedule,
				tr::xgram_MessageDetailsDatePC(tr::now),
			...
			if (view && view->displayedEditDate()) {
				menu2->addAction(Ui::ContextActionWithSubText(
					menu2->menu(),
					st::menuIconEdit,
					tr::xgram_MessageDetailsEditedDatePC(tr::now),
			...
			if (isForwarded) {
				menu2->addAction(Ui::ContextActionWithSubText(
					menu2->menu(),
					st::menuIconTTL,
					tr::xgram_MessageDetailsForwardedDatePC(tr::now),
			...
			if (media && hasAnyMediaField) {
				...
				if (!mediaSize.isEmpty()) {
					menu2->addAction(Ui::ContextActionWithSubText(
						menu2->menu(),
						st::menuIconDownload,
						tr::xgram_MessageDetailsFileSizePC(tr::now),
						mediaSize
					));
				}
				...
				if (!mediaMime.isEmpty()) {
					...
					menu2->addAction(Ui::ContextActionWithSubText(
						menu2->menu(),
						st::menuIconShowAll,
						tr::xgram_MessageDetailsMimeTypePC(tr::now),
						mime.name()
					));
				}
				...
				if (!mediaName.isEmpty()) {
					...
					menu2->addAction(Ui::ContextActionWithSubText(
						menu2->menu(),
						st::xgramEditsHistoryIcon,
						tr::xgram_MessageDetailsFileNamePC(tr::now),
						shortified,
						[=]
						{
				...
				if (!mediaResolution.isEmpty()) {
					menu2->addAction(Ui::ContextActionWithSubText(
						menu2->menu(),
						st::menuIconStats,
						tr::xgram_MessageDetailsResolutionPC(tr::now),
						mediaResolution
					));
				}
				...
				if (!mediaDC.isEmpty()) {
					menu2->addAction(Ui::ContextActionWithSubText(
						menu2->menu(),
						st::menuIconBoosts,
						tr::xgram_MessageDetailsDatacenterPC(tr::now),
						mediaDC
					));
				}
	...
	menu->addAction(
		...
		[=]
		{
			...
			if (useNoQuote) {
				...
				if (XGramForward::isFullXGramForwardNeeded(currentItem)) {
					crl::async([=]
					{
						XGramForward::forwardMessages(session, action, false, resolvedDraft);
					});
				} else if (XGramForward::isXGramForwardNeeded(currentItem)) {
					crl::async([=]
					{
						XGramForward::intelligentForward(session, action, resolvedDraft);
					});
				} else {
...
void AddReadUntilAction(not_null<Ui::PopupMenu*> menu, HistoryItem *item) {
	...
	const auto &ghost = XGramSettings::ghost(&item->history()->session());
	const auto chatGhostMode = XGramSettings::getInstance().isGhostModeInChat(
		item->history()->session().userId().bare,
		item->history()->peer->id.value);
	if (ghost.sendReadMessages() && !chatGhostMode) {
		return;
	}

	menu->addAction(
		tr::xgram_ReadUntilMenuText(tr::now),
		[=]
		{
			readHistory(item);
...
void AddBurnAction(not_null<Ui::PopupMenu*> menu, HistoryItem *item) {
	...
	menu->addAction(
		tr::xgram_ExpireMediaContextMenuText(tr::now),
		[=]
		{
			const auto ids = MTP_vector<MTPint>(1, MTP_int(item->id));
...
void AddCreateFilterAction(not_null<Ui::PopupMenu*> menu,
	...
	menu->addAction(
		tr::xgram_RegexFilterQuickAdd(tr::now),
		[=]
		{
			RegexFilter filter;
