...
#include "xgram/ui/components/saved_music.h"

#include "xgram/xgram_settings.h"
#include "xgram/ui/utils/color_utils.h"
#include "xgram/ui/utils/itunes_search.h"
#include "xgram/ui/utils/palette.h"
...
QRgb AdjustHsl(QRgb color, float luminance, float saturation = -1.0f) {
	auto hsl = XGram::Ui::ColorUtils::colorToHSL(color);
	...
	return XGram::Ui::ColorUtils::HSLToRGB(hsl);
}
...
QPixmap MakeNoCoverImage(const QSize &size) {
	...
	auto image = QImage(size, QImage::Format_ARGB32);
	{
		...
		auto svgIcon = QSvgRenderer(u":/gui/icons/xgram/nocover.svg"_q);
...
std::optional<QRgb> ExtractColorFromCover(const QPixmap &cover) {
	const auto palette = XGram::Ui::Palette::from(cover).generate();
	...
	const auto contrast = XGram::Ui::ColorUtils::calculateContrast(whiteColor, extractedColor);
	...
	if (XGram::Ui::ColorUtils::calculateContrast(whiteColor, adjustedColor) < 3.0f) {
...
XGramMusicButton::XGramMusicButton(
	QWidget *parent,
...
XGramMusicButton::~XGramMusicButton() = default;

void XGramMusicButton::updateData(MusicButtonData data) {
...
void XGramMusicButton::downloadAndMakeCover(FullMsgId msgId) {
...
void XGramMusicButton::makeCover() {
	...
	crl::async(
		[=, mediaView = _mediaView, performerText = _performerText, titleText = _titleText, overrideBg = _overrideBg]()
		{
			...
			if (cover.noCover) {
				const auto pix = XGram::Ui::Itunes::FetchCover(performerText, titleText, size);
				if (!pix.isNull()) {
			...
			crl::on_main([weak, cover = std::move(cover), bgColor, overrideBg]() mutable
			{
				...
				const auto &settings2 = XGramSettings::getInstance();
...
void XGramMusicButton::paintEvent(QPaintEvent *e) {
	if (!_currentCover) {
		return;
	}
...
int XGramMusicButton::resizeGetHeight(int newWidth) {
