...
, _state(lifetime().make_state<State>()) {
	_state->bubbleRadius = XGramSettings::getInstance().messageBubbleRadius();
	...
	const auto xgramUser = HistoryView::GenerateUser(
		history,
		u"XGram Releases"_q);
	...
	rpl::merge(
		XGramSettings::getInstance().replaceBottomInfoWithIconsChanges()
			| rpl::to_empty,
		XGramSettings::getInstance().deletedMarkChanges()
			| rpl::to_empty,
		XGramSettings::getInstance().editedMarkChanges()
			| rpl::to_empty,
		XGramSettings::getInstance().removeMessageTailChanges()
			| rpl::to_empty,
		XGramSettings::getInstance().hideFastShareChanges()
			| rpl::to_empty,
		XGramSettings::getInstance().simpleQuotesAndRepliesChanges()
			| rpl::to_empty,
		XGramSettings::getInstance().semiTransparentDeletedMessagesChanges()
			| rpl::to_empty
	) | rpl::on_next([=] {
		refresh();
	}, lifetime());
...
void MessagePreview::paintEvent(QPaintEvent *e) {
	...
	if (!XGramSettings::getInstance().hideFastShare()) {
