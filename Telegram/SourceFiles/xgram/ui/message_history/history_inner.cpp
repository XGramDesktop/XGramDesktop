...
bool InnerWidget::elementScrollToLocalY(
	...
	_scrollToSignal.fire_copy(top + localTop);
	return true;
}
...
void InnerWidget::preloadMore(Direction direction) {
	...
	crl::async([=] {
		std::vector<XGramMessageBase> messages;
		if (item) { // viewing edited history
			messages = XGramMessages::getEditedMessages(item, minId, maxId, perPage);
		} else { // viewing deleted messages
			messages = XGramMessages::getDeletedMessages(peer, topicId, minId, maxId, perPage, searchQuery);
		}
...
void InnerWidget::addMessages(Direction direction, const std::vector<XGramMessageBase> &messages) {
