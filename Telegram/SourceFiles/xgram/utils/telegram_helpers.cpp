...
#include "xgram/ui/boxes/donate_info_box.h"
#include "xgram/ui/toasts.h"
...
const auto regDateBotFallbackUsername = QString("xgrambot");
	...
	return [=]
	{
		...
		if (isCustomBadge) {
			...
			text = custom.text.isEmpty()
					   ? (isExtera
							  ? tr::xgram_DeveloperPopup(
								  tr::now,
								  lt_item,
								  TextWithEntities{peer->name()},
								  tr::rich)
							  : tr::xgram_SupporterPopup(
								  tr::now,
								  lt_item,
								  TextWithEntities{peer->name()},
								  tr::rich))
			...
			text = peer->isUser()
					   ? tr::xgram_DeveloperPopup(
						   ...
						   TextWithEntities{peer->name()},
						   tr::rich)
					   : tr::xgram_OfficialResourcePopup(
						   tr::now,
						   lt_item,
						   TextWithEntities{peer->name()},
						   tr::rich);
		} else if (isSupporter) {
			text = tr::xgram_SupporterPopup(
				tr::now,
				lt_item,
				TextWithEntities{peer->name()},
				tr::rich);
...
void MarkAsReadThread(not_null<Data::Thread*> thread) {
	...
	XGramWorker::markAsOnline(&thread->session());
}
					 ...
					 [=](Fn<void()> finish)
					 {
						 if (const auto channel = history->peer->asChannel()) {
							 return history->session().api().request(MTPchannels_ReadHistory(
								 ...
								 MTP_int(tillId)
							 )).done([=] { XGramWorker::markAsOnline(&history->session()); }).send();
						 }
						 ...
						 )).done([=](const MTPmessages_AffectedMessages &result)
						 {
							 ...
							 XGramWorker::markAsOnline(&history->session());
						 }).fail([=]
						 {
						 }).send();
					 });
...
void markReadAfterAction(not_null<History*> history) {
	const auto &ghost = XGramSettings::ghost(&history->session());
	if (ghost.sendReadMessages()
		|| !ghost.markReadAfterAction()
		|| XGramSettings::getInstance().isGhostModeInChat(
...
QString formatTTL(int time, bool isDoc) {
	if (time == 0x7FFFFFFF) {
		return isDoc ? tr::xgram_OnePlayTTL(tr::now) : tr::xgram_OneViewTTL(tr::now);
	}
...
void processMessageDelete(not_null<HistoryItem*> item) {
	if (!isMessageSavable(item)) {
		...
		item->setDeleted();
		XGramMessages::addDeletedMessage(item);
	}
}
...
void getUserRegistrationDateInner(
	...
	session->api().request(MTPmessages_GetInlineBotResults(
		...
		for (const auto &entry : data.vresults().v) {
			const auto message = entry.match(
				[](const MTPDbotInlineResult &data) {
					return &data.vsend_message();
				},
				[](const MTPDbotInlineMediaResult &data) {
			...
			if (text.isEmpty() || text == u"failed"_q) {
				continue;
			}
			...
			const auto flag = object[u"flag"_q].toString();
			const auto parsedDate = QDate::fromString(
				object[u"date"_q].toString(),
				u"dd.MM.yyyy"_q);
			if (!parsedDate.isValid()) {
				continue;
			}
			const auto date = TextWithEntities{ langDayOfMonthFull(parsedDate) };
			if (flag == u"EXACT"_q || flag == u"INTERPOLATED"_q) {
				resultText = isSelf
					? tr::xgram_CreationDateSelfApproximately(
						tr::now,
						lt_item,
						date,
						tr::rich)
					: tr::xgram_CreationDateUserApproximately(
						tr::now,
						lt_item1,
						...
						date,
						tr::rich);
			} else if (flag == u"LT"_q) {
				resultText = isSelf
					? tr::xgram_CreationDateSelfEarlier(
						tr::now,
						lt_item,
						date,
						tr::rich)
					: tr::xgram_CreationDateUserEarlier(
						tr::now,
						lt_item1,
						TextWithEntities{ userName },
						lt_item2,
						date,
						tr::rich);
			} else if (flag == u"ET"_q) {
				resultText = isSelf
					? tr::xgram_CreationDateSelfLater(
						tr::now,
						lt_item,
						date,
						tr::rich)
					: tr::xgram_CreationDateUserLater(
						tr::now,
						lt_item1,
						TextWithEntities{ userName },
						lt_item2,
...
void getUserRegistrationDate(
	...
	const auto supporter = isSupporterPeer(selfId) || isExteraPeer(selfId);
	const auto botId = supporter ? regDateBotId : regDateBotFallbackId;
	const auto &botUsername = supporter
		? regDateBotUsername
		: regDateBotFallbackUsername;
	if (session->data().userLoaded(botId)) {
		getUserRegistrationDateInner(user, botId, std::move(callback));
	} else {
		resolvePeer(
...
void getChannelJoinOrCreateDate(not_null<ChannelData*> channel, Fn<void(TextWithEntities)> callback) {
	...
	if (channel->inviteDate) {
		...
		result = tr::xgram_CreationDateChat(
			tr::now,
			lt_item1,
			TextWithEntities{channel->name()},
			lt_item2,
