...
ShareBox::SubmitCallback ShareBox::DefaultForwardCallback(
	...
	return [=](
		...
		if (XGramForward::isFullXGramForwardNeeded(items.front())) {
			crl::async([=]{
				for (const auto thread : result) {
					XGramForward::forwardMessages(
			...
			dismiss();
			return;
		} else if (XGramForward::isXGramForwardNeeded(items)) {
			crl::async([=]
			{
				for (const auto thread : result) {
					XGramForward::intelligentForward(
						&history->owner().session(),
						Api::SendAction(thread, options),
						Data::ResolvedForwardDraft(items, forwardOptions));
				}
