...
protected:
	QSize rightActionSize() const override {
		return isMutualContact()
			? QSize(st::xgramContactsMutualIcon.width(), st::xgramContactsMutualIcon.height())
			: QSize();
	}
	...
	void rightActionPaint(
		...
		(selected || actionSelected
			? st::xgramContactsMutualIconOver
			: st::xgramContactsMutualIcon).paint(p, x, y, outerWidth);
	}
...
object_ptr<Ui::BoxContent> PrepareContactsBox(
	...
	public:
		...
		[[nodiscard]] rpl::producer<int> selectionStateValue() const {
			return _selectionState.value();
		}

		[[nodiscard]] bool selectionMode() const {
			return _selectionState.current() >= 0;
		}
		...
		[[nodiscard]] std::vector<not_null<UserData*>> selectedUsers() const {
		...
		void rowClicked(not_null<PeerListRow*> row) override {
			...
			const auto checked = !_selected.contains(user);
			if (checked) {
				_selected.emplace(user);
			} else {
				_selected.erase(user);
			}
			delegate()->peerListSetRowChecked(row, checked);
			_selectionState = int(_selected.size());
		}
		...
		void rowRightActionClicked(
			...
			if (const auto user = row->peer()->asUser();
				...
				delegate()->peerListUiShow()->showToast(
					tr::xgram_MutualContactInfo(tr::now));
			}
		}

	private:
		rpl::event_stream<not_null<PeerData*>> _wheelClicks;
		rpl::variable<int> _selectionState = -1;
	...
	auto init = [=](not_null<PeerListBox*> box) {
		...
		auto closeText = raw->selectionStateValue(
		) | rpl::map([](int state) {
			return (state >= 0)
				? tr::lng_cancel(tr::now)
				: tr::lng_close(tr::now);
		});
		box->addButton(std::move(closeText), [=] {
			if (raw->selectionMode()) {
				raw->clearSelection();
			} else {
				box->closeBox();
			}
		});
		const auto addContact = box->addLeftButton(
			tr::xgram_AddContactShort(),
			[=] { window->showAddContact(); });
		raw->selectionStateValue(
		) | rpl::on_next([=](int selection) {
			addContact->setVisible(selection < 0);
			box->updateButtonsGeometry();
		}, addContact->lifetime());
		auto deleteText = raw->selectionStateValue(
		) | rpl::map([](int state) {
			return tr::xgram_DeleteSelectedContacts(tr::now)
				+ ((state > 0)
					? (u" ("_q + QString::number(state) + u")"_q)
					: QString());
		});
		box->addButton(std::move(deleteText), [=] {
			if (!raw->selectionMode()) {
				raw->startSelection();
				return;
			}
			const auto selected = raw->selectedUsers();
			if (selected.empty()) {
				window->showToast(tr::xgram_ChooseContacts(tr::now));
				return;
			}
			auto users = QVector<MTPInputUser>();
			users.reserve(selected.size());
			for (const auto user : selected) {
				users.push_back(user->inputUser());
			}
			const auto session = &window->session();
			window->show(Ui::MakeConfirmBox({
				.text = tr::xgram_DeleteSelectedContactsText(),
				.confirmed = [=, users = std::move(users)](
				...
				.confirmText = tr::xgram_DeleteSelectedContacts(),
