...
void SendFilesBox::addMenuButton() {
	...
	top->setClickedCallback([=] {
		...
		if (canSendAsOneTimeMedia()) {
			const auto action = _menu->addAction(
				tr::xgram_SendAsOneTimeMedia(tr::now),
				[=] { _sendAsOneTimeMedia = !_sendAsOneTimeMedia; });
			action->setCheckable(true);
			action->setChecked(_sendAsOneTimeMedia);
		}

		using ImageInfo = Ui::PreparedFileInformation::Image;
		if (_list.files.size() == 1 && std::get_if<ImageInfo>(&_list.files[0].information->media)) {
			_menu->addAction(
				tr::xgram_SendAsSticker(tr::now),
				[=]() mutable
				{
...
bool SendFilesBox::canSendAsOneTimeMedia() const {
	const auto user = _toPeer->asUser();
	if (!user || user->isSelf() || user->isBot() || user->isInaccessible()) {
		return false;
	}
	if (_list.files.size() != 1) {
		return false;
	}
	const auto type = _list.files.front().type;
	return (type == Ui::PreparedFile::Type::Photo)
		|| (type == Ui::PreparedFile::Type::Video);
}
...
void SendFilesBox::send(
	...
	auto way = _sendWay.current();
	if (_sendAsOneTimeMedia && canSendAsOneTimeMedia()) {
		options.ttlSeconds = std::numeric_limits<int>::max();
		// Telegram accepts view-once photos only as photo media, not as a
		// generic document. Force the media path even when the user's default
		// send mode is "as file".
