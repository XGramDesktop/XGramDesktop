...
QImage PeerData::GenerateUserpicImage(
	...
	if (!radius) {
		...
		if (XGramUserpic::ShouldOverrideShape(shape)) {
			radius = XGramUserpic::ComputeRadius(size);
		}
	}
...
bool PeerData::isXGramNoForwards() const {
	if (const auto user = asUser()) {
		return user->isXGramNoForwards();
	} else if (const auto channel = asChannel()) {
		return channel->isXGramNoForwards();
	} else if (const auto chat = asChat()) {
		return chat->isXGramNoForwards();
	}
	return true;
}
