...
, _shortcutMessages(std::make_unique<ShortcutMessages>(this)) {
	...
	rpl::merge(
		XGramSettings::getInstance().showMessageSecondsChanges() | rpl::to_empty,
		XGramSettings::getInstance().replaceBottomInfoWithIconsChanges() | rpl::to_empty,
		XGramSettings::getInstance().deletedMarkChanges() | rpl::to_empty,
		XGramSettings::getInstance().editedMarkChanges() | rpl::to_empty,
		XGramSettings::getInstance().showChannelReactionsChanges() | rpl::to_empty,
		XGramSettings::getInstance().showGroupReactionsChanges() | rpl::to_empty,
		XGramSettings::getInstance().showPrivateChatReactionsChanges() | rpl::to_empty,
		XGramSettings::getInstance().simpleQuotesAndRepliesChanges() | rpl::to_empty,
		XGramSettings::getInstance().hideFastShareChanges() | rpl::to_empty,
		XGramSettings::getInstance().semiTransparentDeletedMessagesChanges() | rpl::to_empty,
		XGramSettings::getInstance().removeMessageTailChanges() | rpl::to_empty
	) | rpl::on_next([=] {
		// These preferences affect cached message view data. Recreate only the
		// currently instantiated views so changes apply to an open chat at once.
		...
		for (const auto item : items) {
			...
			requestItemResize(const_cast<HistoryItem*>(item.get()));
		}
	}, _lifetime);
...
not_null<PeerData*> Session::processChat(const MTPChat &data) {
	...
	data.match([&](const MTPDchat &data) {
		...
		const auto flagsSet = (data.is_left() ? Flag::Left : Flag())
			...
			| (data.is_xgramNoforwards() ? Flag::XGramNoForwards : Flag());
...
void Session::updateEditedMessage(const MTPMessage &data) {
	...
	if (settings.saveMessagesHistory() && !existing->isLocal() && !existing->author()->isSelf() && !edit.isEditHide) {
		...
		XGramMessages::addEditedMessage(existing);
	}
...
std::vector<not_null<HistoryItem*>> Session::allMessages() const {
	auto result = std::vector<not_null<HistoryItem*>>();
	for (const auto &[peerId, messages] : _messages) {
		result.reserve(result.size() + messages.size());
		for (const auto &[messageId, item] : messages) {
...
void Session::unregisterItemView(not_null<ViewElement*> view) {
	...
	if (_heavyViewParts.contains(view)) {
		view->unloadHeavyPart(); // XGram: fix crash when using `saveDeletedMessages`
	}
