...
void Histories::sendReadRequests() {
	...
	// XGram sendReadMessages
	const auto &ghost = XGramSettings::ghost(&_owner->session());
	if (!ghost.sendReadMessages()) {
		DEBUG_LOG(("[XGram] Don't read messages"));
		_states.clear();
		return;
	}
	const auto &settings = XGramSettings::getInstance();
	if (settings.perChatGhostMode()) {
		const auto accountId = _owner->session().userId().bare;
		for (auto i = _states.begin(); i != _states.end();) {
			if (settings.isGhostModeInChat(accountId, i->first->peer->id.value)) {
				i = _states.erase(i);
			} else {
				++i;
			}
	...
	return v::match(message(history, realReplyTo), [&](const auto &request) {
		...
		return sendRequest(history, type, [=](Fn<void()> finish) {
			...
			history->sendRequestId = api->request(
				...
				api->applyUpdates(result, randomId);
				XGramWorker::markAsOnline(&history->owner().session());
