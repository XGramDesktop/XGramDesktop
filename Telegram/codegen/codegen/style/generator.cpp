...
QString Generator::valueAssignmentCode(
	...
	case Tag::Pixels: {
		if (kWideMultiplicable.contains(name)) {
			return QString("XGramUiSettings::getWideMultiplied(%1, %2)").arg(pxValueName(value.Int())).arg(kWideMultiplicable.at(name));
		}
	...
	case Tag::Size: {
		auto v(value.Size());
		if (kWideMultiplicable.contains(name)) {
			return QString("{ XGramUiSettings::getWideMultiplied(%1, %3), XGramUiSettings::getWideMultiplied(%2, %3) }").arg(pxValueName(v.width), pxValueName(v.height)).arg(kWideMultiplicable.at(name));
		}
...
bool Generator::writeIncludesInSource() {
	...
	const auto xgramIncluded = !module_.enumVariables([=](const Variable &value) -> bool
	{
	...
	if (xgramIncluded) {
		source_->include("xgram/xgram_ui_settings.h");
	}
	source_->newline();
	return result;
}
