...
[[nodiscard]] Options RoundOptions(
	...
	return withCorners((radius == ImageRoundRadius::Large)
		...
		: (radius == ImageRoundRadius::Ellipse
			|| radius == ImageRoundRadius::XGramUserpic)
		? Option::RoundCircle
		: Option::None);
}
...
QImage Round(
	...
	if (!static_cast<int>(corners)) {
		return std::move(image);
	} else if (radius == ImageRoundRadius::XGramUserpic) {
		const auto corners_val = XGramUiSettings::getAvatarCorners();
		if (corners_val >= XGramUiSettings::kMaxAvatarCorners) {
		...
		const auto r = int(double(corners_val) / XGramUiSettings::kMaxAvatarCorners * size / 2.0)
