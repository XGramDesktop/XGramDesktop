...
int SwitchShift(not_null<const style::Toggle *> st) {
	return XGramUiSettings::isMaterialSwitches() ? st->shift : st::defaultToggleShift;
}

int SwitchDiameter(not_null<const style::Toggle *> st) {
	return XGramUiSettings::isMaterialSwitches() ? st->diameter : st::defaultToggleDiameter;
}
...
void AbstractCheckView::setChecked(bool checked, anim::type animated) {
	...
	if (animated == anim::type::instant) {
		...
		_toggleAnimation.start(
			...
			_checked ? 0. : 1.,
			_checked ? 1. : 0.,
			XGramUiSettings::isMaterialSwitches() ? _duration : st::defaultToggleDuration,
			XGramUiSettings::isMaterialSwitches() ? anim::easeOutCubic : anim::linear);
	}
...
void ToggleView::paint(QPainter &p, int left, int top, int outerWidth) {
	...
	if (XGramUiSettings::isMaterialSwitches()) {
		const auto xgramToggleAnim = anim::interpolateToF(_st->animPadding, 0, toggled);
		fgRectF.setRect(fgRectF.x() + xgramToggleAnim / 2., fgRectF.y() + xgramToggleAnim / 2., fgRectF.width() - xgramToggleAnim, fgRectF.height() - xgramToggleAnim);
	}
